...
async function ensureSchema() {
  ...
  try {
    ...
    // add is_admin column to existing deployments that predate it
...
// creates demo accounts on startup and keeps passwords in sync with env vars
...
// fills the events table up to 100 rows on first run
...
// start listening immediately so railway health checks don't time out
...
(async () => {
  ...
  // db setup runs async after the server is already listening
...
app.delete("/api/events/:id", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    conn = await pool.getConnection();
    // remove saved references before deleting the event
...
// admin-only route to top up the events table
